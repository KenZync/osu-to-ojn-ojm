...
def convert_to_o2jam(input):
    ...
    with open(os.path.join(os.getcwd(), osu_file), 'r', encoding='utf-8') as f:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith('AudioFilename:'):
                ...
                shutil.copy(filename,str(index)+filename)
    ...
    if is_image(image_file_name):
        ...
        shutil.copy(image_file_name, str(index)+image_file_name)
        image = Image.open(str(index)+"_"+image_file_name)
        ...
        # os.remove(str(index)+image_file_name)
        found_image = True
    if (audio_is_mp3):
